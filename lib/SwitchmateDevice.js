...
var SM1_BATTERY_SERVICE_UUID = '';
var SM1_BATTERY_VALUE_CHAR = '';
...
var SM3_BATTERY_SERVICE_UUID = '0x180F';
var SM3_BATTERY_VALUE_CHAR = '0x2A19';
...
SwitchmateDevice.prototype.onBatteryPercentResult = function(data) {
    if (this._version === 3) {
        if ( data.length > 0 ) {
            this.BatteryState = data;
            this.emit('batterySuccess', data);
        } else {
            this.emit('batteryFail', data);
        }
    } else {
        // To figure out
    }
...
SwitchmateDevice.prototype.BatteryPercent = function(callback) {
    this.onBatteryPercentResultBinded = this.onBatteryPercentResult.bind(this);
    if (this._version === 3) {
        this.notifyCharacteristic(SM3_BATTERY_SERVICE_UUID, SM3_BATTERY_VALUE_CHAR, true, this.onBatteryPercentResultBinded, callback);
    } else {
        // To figure out
        //this.notifyCharacteristic(SM1_BATTERY_SERVICE_UUID, SM1_BATTERY_VALUE_CHAR, true, this.onBatteryPercentResultBinded, callback);
    }
}
